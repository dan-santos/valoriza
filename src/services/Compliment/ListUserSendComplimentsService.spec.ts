...
describe('When list user sendd compliments service is called', () => {
  it('should return list with all compliments sent for the user requester', async() => {
    ...
    // i.e. not all compliments are made by sender

    const listComplimentsService = new ListUserSendComplimentsService(inMemoryComplimentsRepository);
    const complimentsSend = await listComplimentsService.execute(sender.id);

    expect(complimentsSend.length).toStrictEqual(expectedNumberOfCompliments);
  });
